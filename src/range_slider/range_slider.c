...
static uint32_t inline range_slider_get_bar_size(widget_t* widget) {
  ...
  return range_slider->bar_size ? range_slider->bar_size : (range_slider_view_get_height(widget) >> 1);
}
...
static ret_t range_slider_update_label(widget_t *widget, rect_t* fr1, rect_t* fr2) {
  ...
  widget_move_resize(label_max, widget->w - range_label_width, 0, range_label_width, range_label_height);
  widget_move_resize(label_value1, fr1->x + fr1->w - value_label_width / 2, widget->h - value_label_height, value_label_width, value_label_height);
  widget_move_resize(label_value2, fr2->x - value_label_width / 2, widget->h - value_label_height, value_label_width, value_label_height);
...
static ret_t range_slider_on_paint_self(widget_t* widget, canvas_t* c) {
  ...
  return_value_if_fail(RET_OK == range_slider_get_bar_rect(widget, &br, &fr1, &fr2), RET_BAD_PARAMS); 
...
static ret_t range_slider_change_value_by_pointer_event(widget_t* widget, pointer_event_t* evt,
  ...
  if (range_slider->no_dragger_icon) {
    ...
    value = tk_clamp(range * (p.x - half_dragger_size - margin) / (int32_t)(range_slider_view_get_width(widget) - dragger_size - (margin << 1)), 0.0, range);
  }
...
static ret_t range_slider_on_layout_children(widget_t* widget) {
  ...
  char textbuf[10] = {0};
  widget_set_text_utf8(label_min, tk_itoa(textbuf, 10, range_slider->min));
  memset(textbuf, 0, sizeof(textbuf));
  widget_set_text_utf8(label_max, tk_itoa(textbuf, 10, range_slider->max));
  memset(textbuf, 0, sizeof(textbuf));

  widget_move_resize(range_slider_view, widget->w / 2 - range_slider_view_get_width(widget) / 2, 0, range_slider_view_get_width(widget), range_slider_view_get_height(widget));
